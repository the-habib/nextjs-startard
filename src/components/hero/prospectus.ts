...
// Normalize both FLAT and GROUP schemas into a single array of Prospectus
export function extractProspectusFromMessages(messages: any): {
  items: Prospectus[];
  ...
  for (const entry of uniArr) {
    ...
    // GROUP schema: look for tierX arrays
    const keys = Object.keys(p);
    const tierKeys = keys.filter((k) => /^tier[A-Z]$/i.test(k));
    if (tierKeys.length) {
      // Pull optional title/subtitle once
...
export function tiersOf(items: Prospectus[]): string[] {
  return Array.from(new Set(items.map((i) => i.tier))).sort();
}

export function byTier(items: Prospectus[], tier: string): Prospectus[] {
